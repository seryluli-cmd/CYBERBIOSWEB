...
// Admin" en el modal, ver openModal) — pantalla propia, visible solo
// para admin (ver SECCIONES en navegacion.js), para no mezclar lo
// privado con la lista que ven los colaboradores. El total del mes SÍ
// sigue entrando en Resumen mensual (que no filtra nada) — lo único que
// cambia acá es dónde se ve la lista y quién puede verla.
...
// Nada de esto pasa por `state`: es puramente visual y efímero (se
// resetea solo al cambiar de foto o cerrar el visor), no hay ninguna otra
// pantalla que necesite leerlo.
...
// Se cablea una sola vez al arrancar la app (ver app.js) — el visor
// reutiliza siempre el mismo `#visor-fotos-viewport`/`#visor-fotos-img`,
