...
// facturadoUltimosEditados guarda, en orden, los últimos 2 campos que se
// tipearon A MANO (no los que ya se autocompletaron) — con esos 2 se sabe
// cuál es el tercero a calcular. Se reinicia cada vez que se abre el modal
// (ver openModalFacturado()).

const FACTURADO_CAMPO_ID = {
  total: "input-total-fact",
...
function calcularCampoFaltanteFacturado() {
  ...
  const valores = {
    total: parseMoneyInput($("#input-total-fact").value),
...
export function openModalFacturado(cierre, preset) {
  ...
  $("#input-total-fact").value = cierre ? formatMoneyValue(cierre.importe) : "";
